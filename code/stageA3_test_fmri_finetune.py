...
def main(config):
    ...
    output_dir = os.path.join(config.output_root, '%s'%(datetime.datetime.now().strftime("%d-%m-%Y-%H-%M-%S")))
    ...
    # sd['model']['pos_embed'].shape=(1,263,1024)--> num_voxels=262*16=4192 # FIXME why not 4656?
    ...
    if test_set.fmri.shape[-1] < num_voxels:
        ...
        test_set.fmri = test_set.fmri[:, :num_voxels]  # FIXME why ?
    ...
    dataloader = DataLoader(test_set, batch_size=config.batch_size, sampler=sampler)
    ...
    for ep in tqdm(range(config.num_epoch), total=config.num_epoch):
        # if torch.cuda.device_count() > 1: 
        #     sampler.set_epoch(ep) # to shuffle the data at every epoch
        cor = train_one_epoch(model, dataloader, optimizer, device, ep, loss_scaler, logger, config, start_time, model)
        cor_list.append(cor)
        if (ep % 25 == 0 or ep + 1 == config.num_epoch) and ep != 0 :
            # save models
            save_model(config_pretrain, ep, model, optimizer, loss_scaler, os.path.join(output_dir,'checkpoints'))
            # plot figures
            plot_recon_figures(model, ep, device, test_set, output_dir, 5, config, logger, model)
...
@torch.no_grad()
def plot_recon_figures(model, ep, device, dataset, output_dir, num_figures = 5, config=None, logger=None, model_without_ddp=None):
    ...
    output_dir_media = os.path.join(output_dir, 'media')
    os.makedirs(output_dir_media, exist_ok=True)
    fig.savefig(os.path.join(output_dir_media, f'recon_epoch_{ep:04}.png'))
    if logger is not None:
